...
export class CertChainModel {
  ...
  private certChain: cert.X509CertChain | null = null;
  ...
  async createX509CertChain(): Promise<void> {
    ...
    try {
      ...
      this.certChain = null;
      Logger.error(this.tag, `createX509CertChain failed, ${JSON.stringify(err)}`);
    }
  }
  ...
  async validate(hasCrl: boolean): Promise<IResponse> {
    ...
    if (!this.certChain) {
      Logger.error(this.tag, 'validate error, cert chain is unavailable.');
      return {
        code: -1
      };
    }
  ...
  async createValidateParameters(hasCrl: boolean): Promise<cert.CertChainValidationParameters> {
    try {
      ...
      if (hasCrl) {
        const crlEncodingBlob = {
          data: stringToUint8Array(CHAIN_DATA_CRL),
          encodingFormat: cert.EncodingFormat.FORMAT_PEM
        };
        const crl = await cert.createX509CRL(crlEncodingBlob);
        const certCrlCollection = cert.createCertCRLCollection([], [crl]);
        validateParameters.certCRLs = [certCrlCollection];
      }

      return validateParameters;
    } catch (err) {
      Logger.error(this.tag, `createValidateParameters failed, ${JSON.stringify(err)}`);
      throw err;
    }
  }
}
