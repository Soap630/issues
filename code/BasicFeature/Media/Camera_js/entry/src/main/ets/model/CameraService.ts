...
class CameraService {
  ...
  async savePicture(buffer: ArrayBuffer, img: image.Image): Promise<void> {
    try {
      ...
      mediaRequest.addResource(photoAccessHelper.ResourceType.IMAGE_RESOURCE, buffer);
      await this.accessHelper.applyChanges(mediaRequest);
      let imgPhotoUri: string = mediaRequest.getAsset().uri;
      ...
      try {
        ...
        let err = error as BusinessError;
        Logger.error(TAG, `release image err: ${JSON.stringify(err)}`);
      }
    }
  }
  ...
  async initUrl(): Promise<void> {
    let title = `${Date.now()}`;
    this.fileAssetUri = await this.accessHelper.createAsset(photoAccessHelper.PhotoType.VIDEO, 'mp4', { title: title });
    this.fileAsset = await this.getPhotoAssetByUri(this.fileAssetUri);
    let file: fs.File = await fs.open(this.fileAssetUri, fs.OpenMode.READ_WRITE);
    this.fd = file.fd;
    this.videoConfig.url = `fd://${this.fd.toString()}`;
  }

  async getPhotoAssetByUri(uri: string): Promise<photoAccessHelper.PhotoAsset | undefined> {
    if (!uri) {
      return undefined;
    }
    ...
    let fetchResult: photoAccessHelper.FetchResult<photoAccessHelper.PhotoAsset> =
      await this.accessHelper.getAssets(fetchOptions);
    try {
  ...
  async stopVideo(): Promise<photoAccessHelper.PhotoAsset> {
    ...
    try {
      ...
      if (this.fileAsset || this.fileAssetUri) {
        if (!this.fileAsset) {
          this.fileAsset = await this.getPhotoAssetByUri(this.fileAssetUri);
        }
  ...
  async saveDeferredPhoto(proxyObj: camera.DeferredPhotoProxy): Promise<void> {
    try {
      Logger.info(TAG, 'saveDeferredPhoto skip current PhotoAccessHelper resource API');
    } catch (err) {
